...
<div class="row">
		...
		<div class="col-md-8 mt-4">
		 <div class="bg-light profile-div">
		<h3 class="mt-3 ml-2"><c:out value="${user.firstName}"/> <c:out value="${user.lastName}"/>  <button type="button" class="btn btn-sm rounded-pill text-white sign-up" data-toggle="modal" data-target="#exampleModal">
  Contact Info
</button></h3>
		<p class="ml-2"><c:out value="${user.location}"/></p>
		<h4 class="ml-2">About</h4>
		<p class="ml-2"><c:out value="${user.bio}"/></p>
  		</div>
   <div class="bg-light mt-4 pb-5 profile-div">
   <form:form action="/add_post" method="post" class="mt-2" modelAttribute="userpost">
   <form:hidden value="${user.id}" path="user"/>
    ...
    <input type="submit" class="btn sign-up mt-2 pb-2 mr-2 text-white float-right" value="Post"/>
    </div>
</form:form>    
   </div>
    ...
    <div class="bg-light mt-4 profile-div">
				...
				<input type="hidden" name="_method" value="delete"> <input
					type="submit" class="float-right" value="X">
			</form>
    <h5 class="align-top">
    <a class="post-name" href="/profiles/${p.user.id}"><c:out value="${p.user.firstName}"/> <c:out value="${p.user.lastName}"/></a></h5>
